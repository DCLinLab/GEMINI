...
from skimage.filters import difference_of_gaussians, threshold_local, gaussian
...
cpdef frame_detection(cnp.ndarray gfp, cnp.ndarray bf, int thr_blk_sz=31, int tolerance=10, float cutoff_ratio=.1,
    ...
    # gfp = (gfp - (np.mean(gfp) + np.std(gfp))).clip(0)
    ...
    cdef vector[float] new_y, new_x, intensity_list, out_ys, out_xs, out_area
    out_masks = []
    for ys, xs, mask, a in zip(ys_list, xs_list, masks, area):
        if (mask > 0).sum() == 0:
            continue
        y_, x_ = np.nonzero(mask)
        new_y.push_back(y_.mean() + ys)
        new_x.push_back(x_.mean() + xs)
        out_area.push_back(a)
        out_ys.push_back(ys)
        out_xs.push_back(xs)
    ...
    return pd.DataFrame({'y': new_y, 'x': new_x, 'area': out_area, 'intensity': intensity_list,
                         'y_start': out_ys, 'x_start': out_xs}), out_masks
