...
cpdef infuse_patches(cnp.ndarray[cnp.uint16_t, ndim=2] img, y: list[int], x: list[int], masks: list[cnp.ndarray]):
    """
    ...
    cdef cnp.ndarray[cnp.int_t, ndim=2] out = np.zeros_like(img, dtype=int)
    cdef cnp.ndarray[cnp.uint8_t, ndim=2] mask_temp
    cdef int m, n, hm, hn, i, j, i1, j1, c = 0
    for cty, ctx, mask in zip(y, x, masks):
        c += 1
        m, n = mask.shape
        hm = m // 2
        hn = n // 2
